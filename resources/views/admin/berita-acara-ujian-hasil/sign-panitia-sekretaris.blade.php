{{-- filepath: resources/views/admin/berita-acara-ujian-hasil/sign-panitia-sekretaris.blade.php --}}
@extends('layouts.admin.app')

@section('title', 'Tanda Tangan Sekretaris Panitia')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">TTD Sekretaris Panitia</li>
            </ol>
        </nav>
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        ...
                        <h5 class="mb-0">Tanda Tangan Sekretaris Panitia Ujian</h5>
                    </div>
                    <div class="card-body">
                        {{-- Info Berita Acara --}}
                        <div class="alert alert-info mb-4">
                            ...
                            <hr class="my-2">
                            <p class="mb-0"><strong>Judul Skripsi:</strong> {{ $beritaAcara->judul_skripsi }}</p>
                        </div>

                        <form
                            action="{{ route('admin.berita-acara-ujian-hasil.store-sign-panitia-sekretaris', $beritaAcara) }}"
                            method="POST">
                            @csrf
                            ...
                            @if ($isStaff)
                                ...
                                <div class="alert alert-warning mb-4">
                                    ...
                                    <strong>Mode Staff Override:</strong> Anda akan menandatangani atas nama Sekretaris
                                    Panitia (Koordinator Prodi).
                                </div>

                                <div class="mb-3">
                                    <label class="form-label" for="override_korprodi_id">
                                        Pilih Koordinator Prodi <span class="text-danger">*</span>
                                    </label>
                                    <select class="form-select @error('override_korprodi_id') is-invalid @enderror"
                                        id="override_korprodi_id" name="override_korprodi_id" required>
                                        <option value="">-- Pilih Korprodi --</option>
                                        @foreach ($korprodiList as $korprodi)
                                            <option value="{{ $korprodi->id }}"
                                                {{ old('override_korprodi_id') == $korprodi->id ? 'selected' : '' }}>
                                                {{ $korprodi->name }} - {{ $korprodi->jabatan }} (NIP:
                                                {{ $korprodi->nip ?? '-' }})
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('override_korprodi_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    @if ($korprodiList->isEmpty())
                                        <div class="form-text text-warning">
                                            <i class="bx bx-error-circle me-1"></i>
                                            Tidak ada Koordinator Prodi yang terdaftar. Silakan tambahkan user dengan
                                            jabatan "Koordinator Prodi" terlebih dahulu.
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control @error('override_reason') is-invalid @enderror" id="override_reason"
                                        name="override_reason" rows="3"
                                        placeholder="Jelaskan alasan mengapa Anda menandatangani atas nama Korprodi..." required>{{ old('override_reason') }}</textarea>
                                    @error('override_reason')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            @else
                                {{-- Direct Sign Mode (Korprodi) --}}
                                <div class="alert alert-success mb-4">
                                    <i class="bx bx-check-circle me-1"></i>
                                    Anda akan menandatangani sebagai <strong>Sekretaris Panitia Ujian (Koordinator
                                        Prodi)</strong>.
                                </div>
                                ...
                                <div class="mb-4">
                                    <div class="d-flex align-items-center p-3 bg-light rounded">
                                        ...
                                        <div>
                                            ...
                                            <small
                                                class="text-muted">{{ Auth::user()->jabatan ?? 'Koordinator Prodi' }}</small>
                                            <br>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary"
                                    @if ($isStaff && $korprodiList->isEmpty()) disabled @endif>
