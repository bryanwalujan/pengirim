    ...
    <div class="col-12">
        <table class="table table-striped">
            <tbody>
                ...
                <tr>
                    <th>Pembimbing 1</th>
                    <td>{{ $komisi->pembimbing1->name ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <th>Pembimbing 2</th>
                    <td>{{ $komisi->pembimbing2->name ?? 'N/A' }}</td>
                </tr>
                <tr>
...
<div class="row mt-3">
    <div class="col-12">
        <form action="{{ route('admin.komisi-hasil.update-status', $komisi->id) }}" method="POST" class="w-100">
            @csrf
            <div class="mb-3">
        ...
        @if ($komisi->status == 'approved' && $komisi->file_komisi_hasil)
            <a href="{{ route('admin.komisi-hasil.download', $komisi->id) }}" class="btn btn-success w-100 mt-2"
                target="_blank">
