            ...
            <div class="card-header border-bottom">
                <div class="row g-3">
                        ...
                        <div class="col-12 col-md-4 col-lg-3">
                            ...
                            <select class="form-select form-select-sm" id="statusFilter" onchange="filterByStatus(this.value)">
                                ...
                                <option value="diajukan" {{ $status === 'diajukan' ? 'selected' : '' }}>
                                    Diajukan @if(isset($statistics))({{ $statistics['diajukan'] }})@endif
                                </option>
                                <option value="diproses" {{ $status === 'diproses' ? 'selected' : '' }}>
                                    Diproses @if(isset($statistics))({{ $statistics['diproses'] }})@endif
                                </option>
                                <option value="disetujui" {{ $status === 'disetujui' ? 'selected' : '' }}>
                                    Disetujui @if(isset($statistics))({{ $statistics['disetujui'] }})@endif
                                </option>
                                <option value="siap_diambil" {{ $status === 'siap_diambil' ? 'selected' : '' }}>
                                    Siap Diambil @if(isset($statistics))({{ $statistics['siap_diambil'] }})@endif
                                </option>
                            </select>
                        </div>
                    @endif
                    ...
                    <div class="col-12 col-md-8 col-lg-6">
                        ...
                        <form action="{{ route('admin.surat-ijin-survey.index') }}" method="GET" id="searchForm">
                            <div class="input-group input-group-sm">
                                ...
                                <input 
                                    ...
                                    id="searchInput"
                                    value="{{ $search ?? '' }}"
                                    placeholder="Cari nama, NIM, judul, tempat survey..." 
                                    aria-label="Search"
                                />
                                @if (!auth()->user()->hasRole('dosen'))
                            ...
                            <small class="text-muted d-block mt-1">
                                ...
                                Tip: Ketik nama mahasiswa, NIM, judul survey, atau tempat survey
                            </small>
                        </form>
                    </div>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-4">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        @if($search)
                                            <button type="button" class="btn btn-sm btn-outline-primary mt-2" onclick="clearSearch()">
...
@push('scripts')
    <script>
        ...
        function filterByStatus(status) {
            ...
            let url = '{{ route("admin.surat-ijin-survey.index") }}';
