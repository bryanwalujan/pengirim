...
@section('title', 'Kalender Jadwal Ujian Hasil')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
            <div>
                <h4 class="fw-bold mb-1">
                    <i class="bx bx-calendar-event me-2 text-warning"></i>Kalender Jadwal Ujian Hasil
                </h4>
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('admin.jadwal-ujian-hasil.index') }}" class="btn btn-outline-warning">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.jadwal-ujian-hasil.calendar') }}" class="row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('admin.jadwal-ujian-hasil.calendar') }}" class="btn btn-outline-secondary">
                            ...
                            @while ($currentDate <= $endOfCalendar)
                                <tr>
                                    @for ($i = 0; $i < 7; $i++)
                                        ...
                                        @endphp
                                        <td class="calendar-day {{ !$isCurrentMonth ? 'bg-light text-muted' : '' }} {{ $currentDate->isToday() ? 'bg-warning bg-opacity-10' : '' }}"
                                            style="height: 120px; vertical-align: top; position: relative; min-width: 150px;">
                                            <div class="p-2">
                                                <div
                                                    class="fw-bold mb-2 {{ $currentDate->isToday() ? 'text-warning' : '' }}">
                                                    {{ $currentDate->format('d') }}
                                                    @if ($currentDate->isToday())
                                                        <span class="badge bg-warning badge-sm ms-1">Hari Ini</span>
                                                    @endif
                                                </div>
                                                ...
                                                @if ($jadwalsOnDate->isNotEmpty())
                                                    <div class="jadwal-list">
                                                        @foreach ($jadwalsOnDate as $jadwal)
                                                            <div class="jadwal-item mb-2 p-2 bg-warning bg-opacity-10 rounded-2 small cursor-pointer hover-shadow"
                                                                data-bs-toggle="tooltip" data-bs-html="true"
                                                                title="<strong>{{ $jadwal->pendaftaranUjianHasil->user->name }}</strong><br>{{ $jadwal->jam_formatted }}<br>{{ $jadwal->ruangan }}"
                                                                onclick="window.location.href='{{ route('admin.jadwal-ujian-hasil.show', $jadwal) }}'">
                                                                <div class="d-flex align-items-center mb-1">
                                                                    <i class="bx bx-time text-warning me-1"></i>
                                                                    <strong
                                                                        class="text-warning">{{ \Carbon\Carbon::parse($jadwal->waktu_mulai)->format('H:i') }}</strong>
                                                                </div>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    ...
                    <span class="badge bg-warning bg-opacity-10 text-warning">Jadwal Ujian Hasil</span>
