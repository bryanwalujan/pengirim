                ...
                <form action="{{ route('admin.pendaftaran-seminar-proposal.ttd-kajur', $pendaftaran) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-info mb-3">
                            ...
                            <strong>{{ $pendaftaran->suratUsulan->ttd_kaprodi_at->format('d M Y H:i') }} WITA</strong>
                            @if ($pendaftaran->suratUsulan->override_info && isset($pendaftaran->suratUsulan->override_info['kaprodi']))
                                <br><small class="text-muted">
                                    (Override oleh: {{ $pendaftaran->suratUsulan->override_info['kaprodi']['override_by_name'] ?? '-' }})
                                </small>
                            @endif
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" value="{{ $pendaftaran->suratUsulan->nomor_surat }}" readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nama Mahasiswa</label>
                            <input type="text" class="form-control" value="{{ $pendaftaran->user->name }} ({{ $pendaftaran->user->nim }})" readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold">Judul Skripsi</label>
                            <textarea class="form-control" rows="3" readonly>{{ strip_tags($pendaftaran->judul_skripsi) }}</textarea>
                        </div>
