...
// Initialize tooltips & Form Listeners
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Handle Approve On Behalf Form Submission
    const approveOnBehalfForm = document.getElementById("approveOnBehalfForm");
    const approveModalEl = document.getElementById("approveOnBehalfModal");

    if (approveOnBehalfForm && approveModalEl) {
        approveOnBehalfForm.addEventListener("submit", function (e) {
            const nilaiMutuInput = document.getElementById("nilai_mutu");
            const nilaiMutu = nilaiMutuInput.value.trim();

            if (!nilaiMutu) {
                e.preventDefault();
                Swal.fire({
                    title: "Penilaian Wajib!",
                    text: "Sesuai kebijakan terbaru, Anda wajib mengisi nilai mutu saat memberikan persetujuan atas nama dosen.",
                    icon: "warning",
                    confirmButtonColor: "#ffab00",
                    customClass: {
                        container: 'swal2-top-zindex' // Ensure warning is on top
                    }
                }).then(() => {
                    setTimeout(() => nilaiMutuInput.focus(), 300);
                });
                return false;
            }

            // Fix for "tertimpah" (overlapping) issue: 
            // Hide the Bootstrap modal before showing SweetAlert confirmation
            e.preventDefault();
            ...
            Swal.fire({
                ...
                text: "Apakah Anda yakin data penilaian dan persetujuan sudah benar?",
                icon: "question",
                showCancelButton: true,
                confirmButtonText: "Ya, Simpan Persetujuan",
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Memproses...',
                        text: 'Sedang menyimpan persetujuan',
                        allowOutsideClick: false,
                        didOpen: () => {
                            Swal.showLoading();
                        }
                    });
                    approveOnBehalfForm.submit();
                } else {
                    // If cancelled, show the modal again so user can edit
...
function getGradeLetter(nilaiMutu) {
    ...
    if (nilaiMutu >= 3.6) return "A";
    if (nilaiMutu >= 3.0) return "B";
    if (nilaiMutu >= 2.0) return "C";
    if (nilaiMutu >= 1.0) return "D";
    return "E";
}
...
function renderPenilaianSection(penilaian) {
    ...
    if (!penilaian) {
        penilaianContent.innerHTML = createEmptyState(
...
function renderKoreksiSection(dosenId) {
    ...
    if (
        !koreksi ||
        !koreksi.koreksi_data ||
        koreksi.koreksi_data.length === 0
    ) {
        koreksiContent.innerHTML = createEmptyState("Belum ada lembar koreksi");
        return;
    }

    const rows = koreksi.koreksi_data
        .map(
            (item) => `
        <tr>
    ...
    const dateStr = koreksi.created_at
...
function showApproveOnBehalfModal(dosenId, dosenName, posisi) {
    ...
    const modal = new bootstrap.Modal(
        document.getElementById("approveOnBehalfModal"),
    );
    modal.show();
}
...
function updateGradePreview(value) {
    ...
    const grades = [
        { min: 3.6, grade: "A", class: "bg-success", desc: "Sangat Baik" },
        { min: 3.0, grade: "B", class: "bg-info", desc: "Baik" },
        { min: 2.0, grade: "C", class: "bg-warning", desc: "Cukup" },
        { min: 1.0, grade: "D", class: "bg-danger", desc: "Kurang" },
...
const STATUS_LABELS = {
    ...
    menunggu_ttd_panitia_sekretaris: "Menunggu TTD Sekretaris Panitia",
    menunggu_ttd_panitia_ketua: "Menunggu TTD Ketua Panitia",
    selesai: "Selesai",
    ditolak: "Ditolak",
};
