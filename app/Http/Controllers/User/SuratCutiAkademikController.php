...
class SuratCutiAkademikController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $surats = SuratCutiAkademik::with(['status', 'trackings', 'mahasiswa'])
        ...
        return view('user.surat-cuti-akademik.index', [
            'service' => $service,
            'surats' => $surats,
        ]);
    }
    ...
    public function store(SuratCutiAkademikRequest $request)
    {
        ...
        DB::beginTransaction();
        try {
            $surat = SuratCutiAkademik::create([
            ...
            return redirect()->route('user.surat-cuti-akademik.index')
                ->with('success', 'Surat cuti akademik berhasil diajukan');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function show($id)
    {
        ...
        return view('user.surat-cuti-akademik.show', compact('surat'));
    }

    public function confirmTaken($id)
    {
        $surat = SuratCutiAkademik::with(['status'])
            ->where('mahasiswa_id', Auth::id())
            ->where('id', $id)
            ->firstOrFail();
        ...
        try {
            StatusSurat::updateOrCreate(
                ['surat_type' => SuratCutiAkademik::class, 'surat_id' => $surat->id],
                ['status' => 'sudah_diambil', 'updated_by' => Auth::id()]
            );
